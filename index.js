...
const allowedPhoneNumbers = [
  ...
  '6285731055050', //Peggy
  '6285881626656', //Andre
  '6281130569787', //Call center
  '6281352233453', //Mahathir
...
function isTagged(message) {
  const tagPattern = /@(\d+)/; // Regex to match @ followed by numbers (phone numbers)

  const validNumbers = ['6281145401505', '6281130569787'];

  const match = tagPattern.exec(message);
  if (match) {
      const taggedNumber = match[1];

      // Check if the tagged number is exactly one of the valid numbers
...
//   // Check the number of arguments and assign them appropriately
//   if (args.length === 1) {
//       input = args[0]; // Direct call with either username, jobTitle, department, etc.
//   } else if (args.length === 3) {
//       sock = args[0];
//       from = args[1];
//       input = args[2];
...
//           searchCriteria.push(`cn=*${normalizedUsername}*`);
//       }
//       if ('jobTitle' in input) {
//           const normalizedJobTitle = input.jobTitle.toLowerCase();
//           console.log("Searching by job title:", normalizedJobTitle);
...
//     // Check if commandParts has more than 1 part to avoid accessing undefined
//     if (commandParts.length > 1) {
//         const normalizedUsername = commandParts[1].toLowerCase();
//         console.log("Searching by command input username:", normalizedUsername);
//         searchCriteria.push(`cn=*${normalizedUsername}*`);
//     } else {
//         // If the username is missing, log an error and send a message if applicable
//         const errorMessage = 'Error: No username provided with /finduser command';
//         console.error(errorMessage);
//         if (sock && from) {
//             await sock.sendMessage(from, { text: errorMessage });
//         }
...
const handleMessage = async (sock, msg) => {
   ...
   */
  const shouldProcess =
    (!isFromMe && (!isGroupMessage || isTagged(text))) ||
...
//   // Check if the message contains a quoted message
//   const quotedMessage = msg.message.extendedTextMessage?.contextInfo?.quotedMessage;

//   if (quotedMessage) {
//       let combinedMessage = text; // Start with the current message text
//       let quotedText = '';
//       let quotedImageBuffer = null;

//       // Check if the quoted message is a text
//       quotedText = quotedMessage.conversation || quotedMessage.extendedTextMessage?.text || "Unsupported message type or no text found.";
//       console.log('Quoted message text:', quotedText);
//       // if (quotedMessage.conversation) {
//       //     quotedText = quotedMessage.conversation;
//       //     console.log(`Quoted text message: ${quotedText}`);
//       // }
...
//           // Extract the image buffer from the quoted message
//           quotedImageBuffer = await downloadMediaMessage(
...
//           // Analyze the quoted image using your image analysis function
//           const quotedImageAnalysis = await gatherImageInformation(base64QuotedImage, 'Analyze this quoted image.');
//           console.log(`Quoted image analysis result: ${quotedImageAnalysis}`);

//           // Append the quoted image information to the combined message
//           combinedMessage = `Quoted Image Analysis: ${quotedImageAnalysis}\n\nUser: ${text}`;
//       }

//       // If there's quoted text, append it to the combined message
//       if (quotedText) {
//           combinedMessage = `${quotedText}\n\nUser: ${text}`;
//       }

//       console.log(`Combined message for processing: ${combinedMessage}`);

//       // Pass the combined message (with text and/or image analysis) to handleAssistantMessage
...
//                   combinedMessage = `Quoted Image Analysis: Failed to analyze image.\n\nUser: ${text}`;
//               }
//           }
...
app.post(
  ...
  [
    ...
    // message or image (file or URL) must be present
    body('message')
      .trim()
      .custom((v, { req }) => {
        if (!v && !req.file && !req.body.imageUrl) {
          throw new Error('Either message text or image must be provided');
        }
        return true;
      }),
  ],
...
// app.post('/send-group-message', upload.single('image'), [
//   body('id').custom((value, { req }) => {
//       if (!value && !req.body.name) {
//           throw new Error('Invalid value, you can use `id` or `name`');
//       }
...
//   try {
//       if (image) {
//           // If an image is provided, send it with an optional caption
//           const imageBuffer = fs.readFileSync(image.path);
//           await sock.sendMessage(chatId, { image: imageBuffer, caption: message || '' }).then(response => {
//               console.log('Group image message sent:', response);
//               res.status(200).json({
//                   status: true,
//                   response: response
//               });
//               // Delete the uploaded image file after sending
...
//                   response: err.toString()
//               });
//               // Delete the uploaded image file if there's an error
//               fs.unlinkSync(image.path);
//           });
//       } else {
//           // If no image is provided, send a text message
//           await sock.sendMessage(chatId, { text: message }).then(response => {
//               console.log('Group message sent:', response);
//               res.status(200).json({
//                   status: true,
//                   response: response
...
//               document: documentBuffer,
//               //mimetype:'application/pdf',
//               mimetype: doctype,//'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
//               fileName: document.originalname,
//               caption: message || ''
//           }).then(response => {
//               console.log('Group document message sent:', response);
...
//                   response: response
//               });
//               fs.unlinkSync(document.path); // Delete the uploaded document file after sending
//           }).catch(err => {
//               console.error('Error sending group document message:', err);
//               res.status(500).json({
//                   status: false,
//                   response: err.toString()
//               });
//               fs.unlinkSync(document.path); // Delete the uploaded document file if there's an error
//           });
//       } else if (image) {
//           const imageBuffer = fs.readFileSync(image.path);
//           await sock.sendMessage(chatId, { image: imageBuffer, caption: message || '' }).then(response => {
...
//                   response: response
//               });
//               fs.unlinkSync(image.path); // Delete the uploaded image file after sending
//           }).catch(err => {
//               console.error('Error sending group image message:', err);
//               res.status(500).json({
//                   status: false,
//                   response: err.toString()
//               });
//               fs.unlinkSync(image.path); // Delete the uploaded image file if there's an error
//           });
//       } else {
//           await sock.sendMessage(chatId, { text: message }).then(response => {
//               console.log('Group message sent:', response);
...
// // Last Known Good Handle ServiceDesk Update
// app.post('/webhook', checkIP,async (req, res) => {
//   const { id, status, receiver, receiver_type, notify_requester_update = 'false', notify_requester_assign = 'false', notify_technician = 'false' } = req.body;
//   if (!id || !status || !receiver || !receiver_type) {
...
//     // Fetch request details from ServiceDesk Plus
//     const requestObj = await view_request(id); // Use view_request to fetch the request details

//     // Extract relevant details
//     let createdby = requestObj.requester.name;
//     const email = requestObj.requester.email_id;
//     const mobile = requestObj.requester.mobile ? requestObj.requester.mobile : 'N/A'; // Extract mobile number
//     const createdDate = requestObj.created_time.display_value;
//     const subject = requestObj.subject;
//     let description = stripHtmlTagsAndDecode(requestObj.description); // Strip HTML tags and decode entities
//     description = truncateDescription(description); // Truncate description if necessary
//     const workorderid = requestObj.id;
//     const ticketStatus = requestObj.status.name;
//     const priority = requestObj.priority ? requestObj.priority.name : null;
//     const ictTechnician = requestObj.udf_fields ? requestObj.udf_fields.udf_pick_601 : null;
...
//     });
//     if (status === 'new') {
//       console.log('Change incident template from default request to submit a new request');
      
//       try {
//           // Step 1: Define the Service Category based on Request ID
//           const serviceCategory = await defineServiceCategory(workorderid); // Make sure to await this
//           console.log('Determined Service Category:', serviceCategory);
  
//           // Step 2: Update the request with the new service category (template and other details handled in updateRequest)
//           const updateResponse = await updateRequest(workorderid, {
//               serviceCategory,
//               technicianName: ictTechnician, // Assuming ictTechnician is the assigned technician
//               templateId: '305', // Adding templateId
//               templateName: 'Submit a New Request' // Adding templateName
//           });
  
//           console.log('Update Response:', updateResponse);
//           if (!updateResponse.success) {
//               console.log('Failed to update the request:', updateResponse.message);
//               return res.status(500).json({ error: updateResponse.message });
//           }
  
//           // Step 4: Construct the notification message
//           const message = `*New request from ${createdby} on ${createdDate}!*\n*Email:* ${email}\n*Ticket number:* ${workorderid}\n\n*Subject:* ${subject}\n*Description:* ${description}\n\n*For detail, see the link below*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`;
  
//           // Step 5: Send notification to requester if mobile is available
//           if (mobile !== 'N/A') {
//               const formattedMobile = phoneNumberFormatter(mobile);
//               const requesterMessage = `Dear *${createdby}*,\n\nThank you for contacting MTI ICT Helpdesk. Your ticket with subject: "${subject}" has been logged by ticket number *${workorderid}*. Please wait a moment, our support team will reach you soon.\n\n*To monitor your request, please see the link below:*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`;
//               console.log('Sending new request message to requester:', formattedMobile, requesterMessage);
//               // Uncomment the following line to send the message via sock
//               await sock.sendMessage(formattedMobile, { text: requesterMessage });
//           }
  
//           // Step 6: Send the message to the receiver (group or individual)
//           if (receiver_type === 'group') {
//               await sock.sendMessage(receiver, { text: message });
//           } else if (receiver_type === 'individual') {
//               await sock.sendMessage(receiver, { text: message });
//           } else {
//               return res.status(400).json({ error: 'Invalid receiver type' });
//           }
  
//           // Step 7: Store the current state of the ticket for future comparisons
//           await storeCurrentTicketState(workorderid, {
//               status,
//               ticketStatus,
//               priority,
...
//       } catch (error) {
//           console.error('Error processing new request:', error.message);
...
//     else if (status === 'updated') {
//       const previousState = await getPreviousTicketState(workorderid);
//       // Compare current and previous ICT Technician assignment
//       const technicianChanged = (
//         !previousState || // If no previous state, consider it as changed
//         previousState.ictTechnician !== ictTechnician
//       );
//       if (technicianChanged) {
//         // Store the current state of the ticket for future comparisons
//         await storeCurrentTicketState(workorderid, { ictTechnician });
  
//         // Assign technician and group
//         let technician;
//         try {
//           technician = getContactByIctTechnicianName(ictTechnician);
//           if (!technician) {
//             throw new Error('Technician not found');
//           } else {
//             console.log('ICT Technician found:', technician);
//           }
//         } catch (error) {
//           console.error(`Error fetching technician details for ${ictTechnician}:`, error.message);
//           technician = { name: 'Unknown', phone: 'N/A', email: 'N/A', technician: 'Unknown' }; // Default technician details if not found
//         }

//         let groupName;
//         if (technician.technician.includes('IT Support')) {
//           groupName = 'ICT System and Support';
//         } else if (technician.technician.includes('IT Field Support')) {
//           groupName = 'ICT Network and Infrastructure';
//         } else if (technician.technician.includes('Document Control')) {
//           groupName = 'ICT Document Controller';
//         }
...
//             await assignTechnicianToRequest(workorderid, technician.technician, groupName);
//             const formattedMobile = phoneNumberFormatter(mobile);
//             const formattedTechnicianMobile = phoneNumberFormatter(technician.phone);
//             let requesterMessage;
//             let technicianMessage;
//             //If assignment has changed
//             if (previousState) {
//               console.log(`ICT Technician for ticket ${workorderid} has changed from ${previousState.ictTechnician || 'none'} to ${ictTechnician}`);
//               requesterMessage = `Dear *${createdby}*,\n\nYour ticket with subject: "${subject}" has been reassigned from *${previousState.ictTechnician || 'none'}* to *${technician.name}*. Please wait a moment, our support team will reach you soon.\n\n*To monitor your request, please see the link below:*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`;
//             }
//             //If assignment has been assigned 
//             else {
//               console.log(`ICT Technician for ticket ${workorderid} has been assigned to ${ictTechnician}`);
//               requesterMessage = `Dear *${createdby}*,\n\nYour ticket with subject: "${subject}" has been assigned to *${technician.name}*. Please wait a moment, our support team will reach you soon.\n\n*To monitor your request, please see the link below:*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`; 
//             }
//             technicianMessage = `Dear *${technician.name}*,\n\nA new ticket with ID: *${workorderid}* and subject: "${subject}" has been assigned to you. Please check the details and address the request as soon as possible.\n\n*To view the request, please see the link below:*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`;
//             if (notify_requester_assign === 'true') {
//               console.log('Sending message to requester:', formattedMobile, requesterMessage);
//               await sock.sendMessage(formattedMobile, { text: requesterMessage });
//             }
//             if(notify_technician==='true'){
//               console.log('Sending message to technician:', formattedTechnicianMobile, technicianMessage);
//               await sock.sendMessage(formattedTechnicianMobile, { text: technicianMessage });
//             }
...
//       let changes = '';
//       changes += ictTechnician ? `*ICT Technician:* ${ictTechnician}\n` : ''; // Include ICT Technician in updates
//       changes += ticketStatus ? `*Status:* ${ticketStatus}\n` : '';
//       changes += priority ? `*Priority:* ${priority}\n` : '';
//       const message = `*Request update notification!*\n\n*Ticket No.:* ${workorderid}\n*Created by:* ${createdby}\n${changes}\n*Subject:* ${subject}\n\n*Link:* [View Request](https://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1)`;
...
//     res.status(200).json({ message: 'Notification sent successfully' });
//   } catch (error) {
//     console.error('Error fetching request details or sending message:', error);
//     res.status(500).json({ error: 'Failed to send notification' });
//   }
// });

// // Handle ServiceDesk Adding feature assign technician
// app.post('/webhook', async (req, res) => {
//   const { id, status, receiver, receiver_type, notify_requester = 'false', notify_technician = 'false' } = req.body;
//   if (!id || !status || !receiver || !receiver_type) {
//       console.error('Invalid payload:', req.body);
//       return res.status(400).json({ error: 'Invalid payload' });
//   }
...
//       // Fetch request details from ServiceDesk Plus
//       const response = await axios.get(`${base_url}requests/${id}`, { headers, httpsAgent: agent });
//       const requestObj = response.data.request;
...
//       // Always fetch ICT technician details
//       let technician;
//       try {
//           technician = getContactByIctTechnicianName(ictTechnician);
//           if (!technician) {
...
//       // Remove [MTI] from the name
//       createdby = createdby.replace('[MTI]', '').trim();

//       // Check if the technician assignment has changed
//       const technicianAssigned = requestObj.technician ? requestObj.technician.name : null;
//       const newTechnicianAssigned = technician ? technician.name : null;
//       const technicianChanged = technicianAssigned !== newTechnicianAssigned;

//       // Assign technician to request if updated and ICT Technician is assigned
//       if (status === 'updated' && technicianChanged) {
//           let groupName;
//           if (technician.technician.includes('IT Support')) {
//               groupName = 'ICT System and Support';
//           } else if (technician.technician.includes('IT Field Support')) {
//               groupName = 'ICT Network and Infrastructure';
//           } else if (technician.technician.includes('Document Control')) {
//               groupName = 'ICT Document Control';
//           }

//           if (groupName) {
//               console.log(`Assigning technician role ${technician.technician} to group ${groupName} for ticket ${workorderid}`);
//               try {
//                   await assignTechnicianToRequest(workorderid, technician.technician, groupName);

//                   // Notify requester and ICT Technician via WhatsApp if specified
//                   if (notify_requester === 'true') {
//                       const formattedMobile = phoneNumberFormatter(mobile);
//                       const requesterMessage = `Dear *${createdby}*,\n\nYour ticket with subject: "${subject}" has been assigned to *${technician.name}*. Please wait a moment, our support team will reach you soon.\n\n*To monitor your request, please see the link below:*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`;
//                       console.log('Sending message to requester:', formattedMobile, requesterMessage);
//                       await sock.sendMessage(formattedMobile, { text: requesterMessage });
...
//                   if (notify_technician === 'true' && technician.phone !== 'N/A') {
//                       const formattedTechnicianMobile = phoneNumberFormatter(technician.phone);
//                       const technicianMessage = `Dear *${technician.name}*,\n\nA new ticket with subject: "${subject}" has been assigned to you. Please check the details and address the request as soon as possible.\n\n*To view the request, please see the link below:*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`;
//                       console.log('Sending message to technician:', formattedTechnicianMobile, technicianMessage);
//                       await sock.sendMessage(formattedTechnicianMobile, { text: technicianMessage });
//                   }
...
//               }
//           } else {
//               console.log('No valid group found for technician:', technician.technician);
//           }

//       } else {
//           console.log('Technician not assigned or status is not updated.');
//       }

//       // Format the message
//       let message;
//       if (status === 'new') {
//           message = `*New request from ${createdby} on ${createdDate}!*\n*Email:* ${email}\n*Ticket number:* ${workorderid}\n\n*Subject:* ${subject}\n*Description:* ${description}\n\n*For detail, see the link below*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`;

//           // Send a message to the requester if their mobile number is available
//           if (mobile !== 'N/A') {
//               const formattedMobile = phoneNumberFormatter(mobile);
//               const requesterMessage = `Dear *${createdby}*,\n\nThank you for contacting MTI ICT Helpdesk. Your ticket with subject: "${subject}" has been logged by ticket number *${workorderid}*. Please wait a moment, our support team will reach you soon.\n\n*To monitor your request, please see the link below:*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`;
//               console.log('Sending new request message to requester:', formattedMobile, requesterMessage);
...
//           let changes = '';
//           if (status === 'updated') {
//               changes += technician ? `*Technician:* ${technician.technician}\n` : '';
//               changes += ictTechnician ? `*ICT Technician:* ${ictTechnician}\n` : ''; // Include ICT Technician in updates
//               changes += ticketStatus ? `*Status:* ${ticketStatus}\n` : '';
//               changes += priority ? `*Priority:* ${priority}\n` : '';
//           }
//           message = `*Request update notification!*\n\n*Ticket No.:* ${workorderid}\n*Created by:* ${createdby}\n${changes}\n*Subject:* ${subject}\n\n*Link:* [View Request](https://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1)`;

//           // Check the notify_requester flag and send a message to the requester if their mobile number is available
//           if (notify_requester === 'true' && mobile !== 'N/A') {
//               const formattedMobile = phoneNumberFormatter(mobile);
//               const requesterMessage = `Dear *${createdby}*,\n\nYour ticket with subject: "${subject}" has been updated. The current status is as follows:\n\n${changes}\n\n*To monitor your request, please see the link below:*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`;
//               console.log('Sending update message to requester:', formattedMobile, requesterMessage);
//               await sock.sendMessage(formattedMobile, { text: requesterMessage });
//           }

//           // Check the notify_technician flag and send a message to the technician if their mobile number is available
//           if (notify_technician === 'true' && technicianChanged && technician.phone !== 'N/A') {
//               const formattedTechnicianMobile = phoneNumberFormatter(technician.phone);
//               const technicianMessage = `Dear *${technician.name}*,\n\nA new ticket with subject: "${subject}" has been assigned to you. Please check the details and address the request as soon as possible.\n\n*To view the request, please see the link below:*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`;
//               console.log('Sending update message to technician:', formattedTechnicianMobile, technicianMessage);
//               await sock.sendMessage(formattedTechnicianMobile, { text: technicianMessage });
//           }
//       }

//       // Send WhatsApp message based on receiver_type
//       if (receiver_type === 'group') {
//           await sock.sendMessage(receiver, { text: message });
//       } else if (receiver_type === 'individual') {
//           await sock.sendMessage(receiver, { text: message });
...
// //Handle ServiceDesk Update
// app.post('/webhook', async (req, res) => {
//   const { id, status, receiver, receiver_type, notify_requester = 'false' } = req.body;
//   if (!id || !status || !receiver || !receiver_type) {
//       return res.status(400).json({ error: 'Invalid payload' });
//   }
...
//   try {
//       // Fetch request details from ServiceDesk Plus
//       const response = await axios.get(`${base_url}requests/${id}`, { headers, httpsAgent });
//       const requestObj = response.data.request;
...
//       const priority = requestObj.priority ? requestObj.priority.name : null;
//       const ictTechnician = requestObj.udf_fields ? requestObj.udf_fields.udf_pick_601 : null;
//       const technician = getContactByIctTechnicianName(ictTechnician);
//       // Remove [MTI] from the name
//       createdby = createdby.replace('[MTI]', '').trim();
...
//           let changes = '';
//           if (status === 'updated') {
//               changes += technician ? `*Technician:* ${technician}\n` : '';
//               changes += ticketStatus ? `*Status:* ${ticketStatus}\n` : '';
//               changes += priority ? `*Priority:* ${priority}\n` : '';

//           }
//           message = `*Request update notification!*\n\n*Ticket No.:* ${workorderid}\n*Created by:* ${createdby}\n${changes}\n*Subject:* ${subject}\n\n*Link:* [View Request](https://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1)`;
//           //message = `*Request update from ${createdby} on ${createdDate}!*\n*Email:* ${email}\n*Ticket number:* ${workorderid}\n\n*Subject:* ${subject}\n*Description:* ${description}\n\n*For detail, see the link below*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1)`;
//             // Check the notify_requester flag and send a message to the requester if their mobile number is available
//           if (notify_requester === 'true' && mobile !== 'N/A') {
//               const formattedMobile = phoneNumberFormatter(mobile);
//               const requesterMessage = `Dear *${createdby}*,\n\nYour ticket with subject: "${subject}" has been updated. The current status is as follows:\n\n${changes}\n\n*To monitor your request, please see the link below:*\nhttps://helpdesk.merdekabattery.com:8080/WorkOrder.do?woMode=viewWO&woID=${workorderid}&PORTALID=1`;
